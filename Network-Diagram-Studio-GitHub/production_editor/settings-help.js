...
(function () {
  ...
  const copy = {
    en: {
      ...
      tabs: ['Tabs', 'Each tab holds its own diagram, shapes and notes. Drag tabs to reorder, or focus a tab and use Alt+Left/Right. The browser remembers the order. Save JSON writes only the current tab.'],
    ...
    ja: {
      ...
      tabs: ['タブ', '各タブには、それぞれの図、図形、メモが保存されます。ドラッグするか、タブにフォーカスして Alt+左右矢印を押すと並べ替えられます。順序はブラウザーに保存されます。JSON 保存の対象は現在のタブだけです。'],
    ...
    zh: {
      ...
      tabs: ['标签页', '每个标签页都有自己的关系图、形状和笔记。拖动标签页，或聚焦标签页后按 Alt+左/右方向键即可排序。浏览器会记住顺序。保存 JSON 只保存当前标签页。'],
    ...
    es: {
      ...
      tabs: ['Pestañas', 'Cada pestaña tiene su propio diagrama, formas y notas. Arrastra las pestañas para reordenarlas o enfoca una y usa Alt+Izquierda/Derecha. El navegador recuerda el orden. Guardar JSON solo guarda la pestaña actual.'],
    ...
    fr: {
      ...
      tabs: ['Onglets', 'Chaque onglet possède son schéma, ses formes et ses notes. Faites glisser les onglets pour les réordonner, ou placez le focus sur un onglet et utilisez Alt+Gauche/Droite. Le navigateur mémorise l’ordre. Enregistrer JSON enregistre uniquement l’onglet actuel.'],
    ...
    de: {
      ...
      tabs: ['Tabs', 'Jede Registerkarte enthält ihr eigenes Diagramm, Formen und Notizen. Zum Sortieren ziehen oder eine Registerkarte fokussieren und Alt+Pfeil links/rechts verwenden. Der Browser merkt sich die Reihenfolge. JSON speichern speichert nur die aktuelle Registerkarte.'],
    ...
    pt: {
      ...
      tabs: ['Guias', 'Cada guia tem seu próprio diagrama, formas e notas. Arraste as guias para reordenar ou foque uma guia e use Alt+Esquerda/Direita. O navegador memoriza a ordem. Salvar JSON salva apenas a guia atual.'],
    ...
    ko: {
      ...
      tabs: ['탭', '각 탭에는 자체 다이어그램, 도형과 메모가 있습니다. 탭을 드래그하거나 탭에 포커스를 두고 Alt+왼쪽/오른쪽 화살표로 순서를 바꾸세요. 브라우저가 순서를 기억합니다. JSON 저장은 현재 탭만 저장합니다.'],
